...
#define COMPUTE_EYEDEPTH(o) o = -UnityObjectToViewPos( v.vertex + (v.color * _Distance) ).z
#define COMPUTE_DEPTH_01 -(UnityObjectToViewPos( v.vertex + (v.color * _Distance) ).z * _ProjectionParams.w)
...
float4 UnityApplyLinearShadowBias(float4 clipPos)
{
    ...
    #define TRANSFER_SHADOW_CASTER_NOPOS_LEGACY(o,opos) o.vec = mul(unity_ObjectToWorld, v.vertex + (v.color * _Distance)).xyz - _LightPositionRange.xyz; opos = UnityObjectToClipPos(v.vertex + (v.color * _Distance));
    #define TRANSFER_SHADOW_CASTER_NOPOS(o,opos) o.vec = mul(unity_ObjectToWorld, v.vertex + (v.color * _Distance)).xyz - _LightPositionRange.xyz; opos = UnityObjectToClipPos(v.vertex + (v.color * _Distance));
    ...
    #define TRANSFER_SHADOW_CASTER_NOPOS_LEGACY(o,opos) \
        opos = UnityObjectToClipPos(v.vertex.xyz + (v.color * _Distance)); \
        opos = UnityApplyLinearShadowBias(opos);
    #define TRANSFER_SHADOW_CASTER_NOPOS(o,opos) \
        opos = UnityClipSpaceShadowCasterPos(v.vertex + (v.color * _Distance), v.normal); \
        opos = UnityApplyLinearShadowBias(opos);
...
#define TRANSFER_SHADOW_COLLECTOR(o)    \
    o.pos = UnityObjectToClipPos(v.vertex + (v.color * _Distance)); \
    float4 wpos = mul(unity_ObjectToWorld, v.vertex + (v.color * _Distance)); \
    o._WorldPosViewZ.xyz = wpos; \
    o._WorldPosViewZ.w = -UnityObjectToViewPos(v.vertex + (v.color * _Distance)).z; \
